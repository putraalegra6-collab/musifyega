var Library={
    ...
    render(){
        ...
        var html = '<div class="pt-8 pb-3.5 px-4 sticky top-0 z-30 border-b border-white/10 shadow-2xl transition-all" style="background: linear-gradient(180deg, rgba(232, 120, 159, 0.12) 0%, rgba(13, 13, 12, 0.45) 55%, rgba(13, 13, 12, 0.92) 100%); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);">' +
        ...
        if (isArtistsTab) {
            if(likedArtists.length === 0){
                ...
                likedArtists.forEach(function(a){
                    html += '<div onclick="Artist.open(\'' + es(a.artistId) + '\', \'' + esJs(a.name) + '\')" class="p-3.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] cursor-pointer active:scale-95 transition-all text-center flex flex-col items-center justify-center group">' +
            ...
            if(pls.length === 0){
                ...
                pls.forEach(function(p){
                    html += '<div onclick="Library.open(\'' + p.id + '\')" class="p-2.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] cursor-pointer active:scale-95 transition-all group flex flex-col">' +
    ...
    open(id){
        ...
        if(!modal) {
            ...
            modal.className = 'fixed inset-0 bg-[#0d0d0c] flex flex-col z-[100]';
            modal.style.animation = 'slideUp 0.3s ease-out forwards';
            document.body.appendChild(modal);
        }
        modal.style.display = 'flex';
        var html='<div class="flex items-center gap-3 p-4 pt-safe bg-transparent absolute top-0 left-0 w-full z-[100] transition-all" id="library-header">'+
