var Album = {
    init() {
        ...
        gid('album-container').innerHTML = '<div id="album-modal" class="fixed inset-0 bg-[#0d0d0c] flex flex-col z-[100]" style="display:none; animation: slideUp 0.3s ease-out forwards;">'+
            ...
            '<div class="flex-1 overflow-y-auto hide-scrollbar pb-36 relative" id="album-content" onscroll="Album.handleScroll()">'+
                '<div class="flex justify-center mt-32">'+
                    '<div class="w-10 h-10 border-3 border-[#e8d6c4] border-t-transparent rounded-full animate-spin"></div>'+
                '</div>'+
            '</div>'+
        '</div>';
        lucide.createIcons();
    ...
    handleScroll() {
        ...
        if (!h) return;
        if (c.scrollTop > 50) {
            h.style.background = 'rgba(13, 13, 12, 0.92)';
        } else {
    ...
    open(id, passedCoverUrl) {
        ...
        gid('album-content').innerHTML = '<div class="flex justify-center mt-32">'+
            '<div class="w-10 h-10 border-3 border-[#e8d6c4] border-t-transparent rounded-full animate-spin"></div>'+
        '</div>';
        ...
        .then(data => {
            ...
            let html = '<div class="relative w-full aspect-square md:aspect-video max-h-[50vh] overflow-hidden -mt-20 mb-6">'+
                '<img src="${im}" class="w-full h-full object-cover" />'+
                '<div class="absolute inset-0 bg-gradient-to-t from-[#0d0d0c] via-[#0d0d0c]/60 to-transparent"></div>'+
            ...
            if(a.songs && a.songs.length > 0) {
                ...
                a.songs.forEach((s, i) => {
                    ...
                    if (!sim) sim = im;
                    var isCur = S.ct && (S.ct.id === s.videoId || S.ct.videoId === s.videoId || (S.ct.title === s.title && S.ct.artist === s.artist));
                    ...
                    html += '<div data-lp-album="${id}" data-lp-index="${i}" onclick="Album.playSong(\'${id}\', ${i})" class="flex items-center gap-3 p-2.5 rounded-2xl cursor-pointer group active:scale-[0.98] transition-all duration-300 shadow-lg ${rowBg}">'+
    ...
    share(id) {
        ...
        var artist = a.artist || '';
        var url = location.origin + '/album/' + targetId + '?title=' + encodeURIComponent(title) + '&artist=' + encodeURIComponent(artist) + '&cover=' + encodeURIComponent(cover);
        if (typeof updateOGForAlbum === 'function') {
            updateOGForAlbum(title, cover, artist);
        }
        if (navigator.share) {
            navigator.share({
                title: title + ' - MusifyEga',
                text: 'Dengarkan album ' + title + ' di MusifyEga!',
                url: url
            }).catch(function() {});
        } else {
