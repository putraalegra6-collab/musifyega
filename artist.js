var Artist={
    init(){
        gid('artist-container').innerHTML='<div id="artist-modal" class="fixed inset-0 bg-[#141413] flex flex-col z-30" style="display:none; animation: slideUp 0.3s ease-out forwards;">'+
            ...
            '<div class="flex-1 overflow-y-auto hide-scrollbar pb-36 relative" id="artist-content" onscroll="Artist.handleScroll()">'+
                '<p class="text-center text-[#9a9490] mt-32">Memuat...</p>'+
            '</div>'+
        '</div>';
        lucide.createIcons();
    },
    handleScroll() {
        ...
        if (!h) return;
        if (c.scrollTop > 50) {
            h.style.background = 'rgba(20, 20, 19, 0.92)';
    ...
    open(id, name, thumbnail) {
        ...
        gid('artist-content').innerHTML = '<div class="relative w-full aspect-square max-h-[40vh] overflow-hidden -mt-16 mb-6">'+
            '<img src="${tImg}" class="w-full h-full object-cover opacity-50" onerror="this.src=\'${FI}\'" />'+
            '<div class="absolute inset-0 bg-gradient-to-t from-[#141413] to-transparent"></div>'+
        ...
        '<div id="artist-inner-content">'+
            '<div class="flex justify-center mt-12">'+
                '<div class="w-8 h-8 border-3 border-[#e8d6c4] border-t-transparent rounded-full animate-spin"></div>'+
            '</div>'+
        '</div>';
        fetch(API.artist + '?id=' + id).then(function(r){return r.json();}).then(function(d){
            if(d.status && d.result){
                ...
                var fullHtml = '<div class="relative w-full aspect-square max-h-[50vh] overflow-hidden -mt-20 mb-6">'+
                    '<img src="${headerImg}" class="w-full h-full object-cover" onerror="this.src=\'${FI}\'" />'+
                    '<div class="absolute inset-0 bg-gradient-to-t from-[#141413] via-[#141413]/60 to-transparent"></div>'+
                    '<div class="absolute bottom-6 left-6 right-6 flex flex-col justify-end items-center text-center">'+
                ...
                if(a.topSongs&&a.topSongs.length>0){
                    ...
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Lagu Teratas</h3><div id="artist-songs-list" class="space-y-1 px-2">';
                    a.topSongs.slice(0,10).forEach(function(s,i){
                        ...
                        var isCur = S.ct && (S.ct.id === s.videoId || S.ct.videoId === s.videoId || (S.ct.title === s.title && S.ct.artist === (s.artist||a.name)));
                        ...
                        var rowBg = isPlay ? 'bg-[#433e3b] border border-white/40 shadow-xl' : (isCur ? 'bg-[#3a332f] border border-white/30' : 'bg-[#2a2521] border border-white/10 hover:bg-[#363230]');
                        var titleClass = isCur ? 'text-white font-bold' : 'text-white/90 font-semibold';
                        html+='<div data-lp-artist="1" data-lp-index="${i}" onclick="Artist.play(\'${s.videoId}\',\'${esJs(s.title)}\',\'${esJs(s.artist||a.name)}\',\'${esJs(im)}\')" class="flex items-center gap-3 p-2.5 rounded-2xl cursor-pointer active:scale-[0.98] transition-all duration-300 shadow-lg group ${rowBg}">'+
                ...
                if(a.topAlbums&&a.topAlbums.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Album</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.topAlbums.forEach(function(al){
                        ...
                        html+='<div onclick="Album.open(\'${al.browseId}\', \'${esJs(im)}\')" class="flex-shrink-0 w-36 cursor-pointer group p-2 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] transition-all">'+
                ...
                if(a.topSingles&&a.topSingles.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Singles & EP</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.topSingles.forEach(function(sg){
                        ...
                        html+='<div onclick="Album.open(\'${sg.browseId}\', \'${esJs(im)}\')" class="flex-shrink-0 w-36 cursor-pointer group p-2.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] transition-all flex flex-col">'+
                            '<div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">'+
                                '<img src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src=\'${FI}\'" />'+
                            '</div>'+
                ...
                if(a.topVideos&&a.topVideos.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Video</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.topVideos.forEach(function(vd){
                        ...
                        html+='<div onclick="Artist.play(\'${vd.videoId||\'\'}\',\'${esJs(vd.name)}\',\'${esJs(vd.artist||a.name)}\',\'${esJs(im)}\')" class="flex-shrink-0 w-48 cursor-pointer group p-2.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] transition-all flex flex-col">'+
                ...
                if(a.playlists&&a.playlists.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Playlist</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.playlists.forEach(function(pl){
                        ...
                        html+='<div onclick="Album.open(\'${pl.browseId}\', \'${esJs(im)}\')" class="flex-shrink-0 w-36 cursor-pointer group p-2.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] transition-all flex flex-col">'+
                            '<div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">'+
                                '<img src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src=\'${FI}\'" />'+
                            '</div>'+
                ...
                if(a.featuredOn&&a.featuredOn.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Tampil Di</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.featuredOn.forEach(function(fo){
                        ...
                        html+='<div onclick="Album.open(\'${fo.browseId}\', \'${esJs(im)}\')" class="flex-shrink-0 w-36 cursor-pointer group p-2.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] transition-all flex flex-col">'+
                            '<div class="w-full aspect-square rounded-xl overflow-hidden mb-2 shadow-md">'+
                                '<img src="${im}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" onerror="this.src=\'${FI}\'" />'+
                            '</div>'+
                ...
                if(a.similarArtists&&a.similarArtists.length>0){
                    html+='<div class="mb-6"><h3 class="font-bold text-sm text-[#9a9490] uppercase tracking-wider mb-3 px-4">Artis Serupa</h3><div class="flex gap-3 overflow-x-auto hide-scrollbar pb-2 px-4">';
                    a.similarArtists.forEach(function(s){
                        ...
                        html+='<div onclick="Artist.open(\'${s.browseId}\',\'${esJs(s.name)}\')" class="flex-shrink-0 w-32 cursor-pointer group p-2.5 rounded-2xl bg-[#2a2521] border border-white/10 shadow-xl hover:bg-[#363230] transition-all flex flex-col items-center">'+
    ...
    share() {
        ...
        var url = location.origin + '/artist/' + Artist.currentArtistId + '?name=' + encodeURIComponent(name) + '&cover=' + encodeURIComponent(cover);
        if (typeof updateOGForArtist === 'function') {
            updateOGForArtist(name, cover);
        }
        if (navigator.share) {
            navigator.share({
                title: name + ' - MusifyEga',
                text: 'Dengarkan lagu & album terbaik dari ' + name + ' di MusifyEga!',
                url: url
            }).catch(function() {});
        } else {
